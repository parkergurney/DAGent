...
Authentication is injected at run time through Harbor's host-environment
template. The task package contains no literal API key or OAuth token. The
launcher bind-mounts the mode-600 auth file by path, and the in-container
runtime parses it without invoking a shell. This keeps the token out of
Harbor's host environment and out of `docker compose exec -e` arguments. To
use a subscription token generated by `claude setup-token`, place it in a
mode-600 env file and set `ORCH_AUTH_ENV_FILE` to that path. Do not commit the
env file or pass the token as a command-line argument.
