...
fault_task=${ORCH_BENCHMARK_FAULT_TASK:-schema}
...
validate_fault() {
  job=$1
  .venv/bin/python - "$job" "$fault_task" <<'PY'
import json
import pathlib
import sys
...
run_cell() {
  ...
  seed=$3
  echo
  echo "== $kind / $policy / seed $seed =="
  start=$(date +%s)
  if [ "$kind" = "clean" ]; then
    ORCH_FAULT_TASK= ORCH_TARGET_REACHABLE=0 \
      ORCH_BENCHMARK_MODEL="$model" \
      "$launcher" "$policy" "$seed" "$model"
  else
    ORCH_FAULT_TASK="$fault_task" \
...
for seed in $seeds; do
  ...
  for policy in sequential naive-parallel orchestrator; do
    run_cell clean "$policy" "$seed"
  done
  for policy in sequential naive-parallel orchestrator; do
    run_cell fault "$policy" "$seed"
  done
done

echo
echo "benchmark complete: seeds=$seeds model=$model fault_target=$fault_task"
