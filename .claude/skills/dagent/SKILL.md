...
below. `dagent run`/`daemon` themselves spawn real Claude Code worker
...
```
dagent add-task --repo <path> --title "<short title>" --brief "<what to do>" \
...
```
dagent run --repo-root <path> [--external-isolation for Harbor/container
...
```
dagent daemon --repo-root <path> [--poll-interval <seconds>]
...
one that's stuck" without an id, run `dagent status` first to find it
rather than guessing.

All commands take `--db <path>` (default `data/dagent.db`); only pass
...
- `pr`: run `dagent status <task_id>` and give them the PR URL, branch,
  and commit SHA. The PR is the review surface.
- `local`: run `dagent status <task_id>` and give them the printed
...
5. **`daemon` requires an explicit, separate confirmation every time**,
   ...
   - Launch it with Bash `run_in_background: true` (it never exits, so this
     ...
     `TaskStop` can end it cleanly), or `pkill -f "dagent daemon"`
...
6. **Surface `needs_human` proactively, don't just report state.** A
   ...
   `dagent status <task_id>` for the full escalation and relay its
