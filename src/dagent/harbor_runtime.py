"""In-container entry point used by :mod:`dagent.harbor_agent`.
...
from dagent.harbor import export_patch, export_task_summary, run_instruction
from dagent.metrics import export_metrics
from dagent.experiment import classify_cell
from dagent.diagnostics import live_diagnostic
...
def _package_version() -> str:
    try:
        return importlib.metadata.version("dagent")
...
async def run_from_files(instruction_file: str | Path, config_file: str | Path | None = None) -> int:
    ...
    run_artifact_root = Path(_value(
        ...
        f"/tmp/dagent-artifacts-{os.getpid()}",
    ))
    ...
    db_path = _value(settings, "db_path", "ORCH_DB_PATH", "/tmp/dagent.db")
    worktree_root = _value(
        settings, "worktree_root", "ORCH_WORKTREE_ROOT", "/tmp/dagent-worktrees"
    )
